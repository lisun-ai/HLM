...
class DisentangledSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        #print(33, self.all_head_size, self.num_attention_heads, self.attention_head_size) # 1020 12 85
        #print(44, config.hidden_size, config.num_attention_heads, self.attention_head_size) # 1024 12 85
        self.query_proj = nn.Linear(config.hidden_size, self.all_head_size, bias=True)
        self.key_proj = nn.Linear(config.hidden_size, self.all_head_size, bias=True)
        self.value_proj = nn.Linear(config.hidden_size, self.all_head_size, bias=True)

        self.share_att_key = getattr(config, 'share_att_key', False)
        self.fp32_attention = getattr(config, 'fp32_attention', True)
    ...
    def forward(self, hidden_states, attention_mask, return_att=False, query_states=None, relative_pos=None, \
        rel_embeddings=None, prev_states=None, history_states=None, tag=None, **kwargs):
        _query_states = query_states if query_states is not None else hidden_states
        tag = tag + '_' if tag else ''
        query_layer = self.transpose_for_scores(self.query_proj(_query_states), self.num_attention_heads)
        if history_states is not None:
            _hidden_states = torch.cat([history_states, hidden_states], dim=1)
        else:
            _hidden_states = hidden_states

        if prev_states is not None:
            # Prev_states used for recurrent inference
            # K, V
            # Mask - Optional
            # Attention logits - Optional
            cached_key = prev_states.get(tag + 'key', None)
            cached_value = prev_states.get(tag + 'value', None)
        else:
            cached_key = cached_value = None
        if (query_states is None) or (cached_key is None) or (cached_value is None):
            key_layer = self.transpose_for_scores(self.key_proj(_hidden_states), self.num_attention_heads)
            value_layer = self.transpose_for_scores(self.value_proj(_hidden_states), self.num_attention_heads)

        # bhsd
        if (cached_key is not None) and (cached_value is not None):
            if (key_layer is not None):
                key_layer = torch.cat([cached_key, key_layer], dim=-2)
                value_layer = torch.cat([cached_value, value_layer], dim=-2)
            else:
                key_layer = cached_key
                value_layer = cached_value

        if prev_states is not None:
            prev_states[tag + 'key'] = key_layer
            prev_states[tag + 'value'] = value_layer
        ...
        if self.fp32_attention:
            attention_scores = torch.bmm(query_layer.float(), key_layer.transpose(-1, -2).float()*scale)
        else:
        ...
        if self.relative_attention:
            ...
            if self.fp32_attention:
                rel_att = self.disentangled_attention_bias(query_layer.float(), key_layer.float(), relative_pos, rel_embeddings, scale_factor)
            else:
